...
#define FP_POPOVER_ARROW_HEIGHT 15.0//调节箭头高度
#define FP_POPOVER_ARROW_BASE 20.0//调节箭头下边的宽度
#define FP_POPOVER_RADIUS 10.0//调节边框的圆润度
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        //_titleLabel.textAlignment = UITextAlignmentCenter;//clear
...
-(CGPathRef)createContentPathWithBorderWidth:(CGFloat)borderWidth arrowDirection:(FPPopoverArrowDirection)direction
{
    ...
    CGFloat h = self.bounds.size.height;//可以设置页面的高度
    ...
    CGPathMoveToPoint(path, nil, innerRect.origin.x, outside_top);
    //top arrow
    if(direction == FPPopoverArrowDirectionUp)
    {
...
- (void)drawRect:(CGRect)rect
{
    ...
    CGGradientRef gradient1 = [self createGradient];
    ...
    CGContextDrawLinearGradient(ctx,gradient1,start,end, 0);
...
-(void)setupViews
{
    ...
    if(_arrowDirection == FPPopoverArrowDirectionUp)
    {
        contentRect.origin = CGPointMake(10, 30);//设置view距离上边框的高度
        contentRect.size = CGSizeMake(self.bounds.size.width-20, self.bounds.size.height-40);//设置view的大小
